    ...
    public partial class Form1 : Form
    {
        ...
        private void ReadAndShowScore()
        {
            using (StreamReader reader = new StreamReader("Score.ini"))
            {
                ...
                if (score == int.Parse(label1.Text))
                {
                    MessageBox.Show(string.Format("Ai facut exact {0}.(scorul maxim) . Incearca sa-l depasesti de data asta",score), "Flappy Bird", MessageBoxButtons.OK, MessageBoxIcon.Information); 
                }
            }
        }
        ...
        private void Form1_Paint(object sender, PaintEventArgs e)
        {
            if(!resetPipes && Pipe1.Any() && Pipe2.Any())
            {
                ...
                e.Graphics.FillRectangle(Brushes.DarkGreen, new Rectangle(Pipe2[0] - 10, Pipe1[3] , 75, 15));
                //a doua de sus
        ...
        private void checkforpoints()
        {
            ...
            Rectangle rect1 = new Rectangle(Pipe1[2] + 20, Pipe1[3] - PipeDifferentY, 15, PipeDifferentY);
            Rectangle rect2 = new Rectangle(Pipe2[2] + 20, Pipe2[3] - PipeDifferentY, 15, PipeDifferentY);

            Rectangle intersect1 = Rectangle.Intersect(rect, rect1);
            Rectangle intersect2 = Rectangle.Intersect(rect, rect2);
            ...
            if (!resetPipes | start)
            {
                if(intersect1!=Rectangle.Empty | intersect2 != Rectangle.Empty)
                {
                    ...
                    {
                        points++;
                        SoundPlayer sp = new SoundPlayer(WindowsFormsApp1.Properties.Resources.point);
                        sp.Play();
                        inPipe = true;
        ...
        private void CheckForCollision()
        {
            ...
            Rectangle intersect1 = Rectangle.Intersect(rect, rec1);
            Rectangle intersect2 = Rectangle.Intersect(rect, rec2);
            Rectangle intersect3 = Rectangle.Intersect(rect, rec3);
            Rectangle intersect4 = Rectangle.Intersect(rect, rec4);

            if (!resetPipes | start)
            {
                if (intersect1 != Rectangle.Empty | intersect2 != Rectangle.Empty | intersect3 != Rectangle.Empty | intersect4 != Rectangle.Empty)
                {
                    SoundPlayer sp = new SoundPlayer(WindowsFormsApp1.Properties.Resources.collision);
                    sp.Play();
                    Die();
                }
            }
